...
def weather_data_query(city, app_id, date, **kwargs):
    ...
    if new_req.validate_app_id() is not True:
        app.logger.error('ERROR 401 - Unauthorised request using invalid API key (app_id): %s', app_id)
        response = {
                        'status': '401',
        ...
        app.logger.info('SUCCESS 200 - Valid API key used: %s', app_id)
        return jsonify(
            {
...
class WeatherAPI(Resource):
    def get(self):
        ...
        parser.add_argument('app_id',
                            ...
                            required=True,
                            location='args',
                            help='ERROR 400 - Missing an app_id. Get one by registering here: http://openweathermap.org')
        parser.add_argument('date',
                            type=str,
                            required=True,
                            location='args',
                            help='ERROR 400 - Please provide YYYYMMDD date string')
